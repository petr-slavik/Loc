...
#line length = 92
#length limit for trados = 71 chars

source_file = "00_source\\SIMGHELP.NCT"
target_file = "00_source\\SIMGHELP_target.NCT"
new_target_file = "00_source\\SIMGHELP_target_translated.NCT"

pattern1 = r"\s*\d\d\s{15}E$"
pattern2 = r"^Z\s+"
pattern = r"(^Z\s+)(.*?)(\s*\d\d\s{15}E$)"
    ...
    with open(new_target_file, "w",  encoding="utf8", errors="ignore") as new_target:
        for x, line in enumerate(target, 0):
            if line.startswith("\r\n"):
                ...
                while (len(new_line) + 1) < (len(source[x]) - 17):
                    new_line += " "
